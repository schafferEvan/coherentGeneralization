...
global Ny classLevel2 classLevel1
...
No = 1000; %200;
dSteps = 1e5;%round(logspace(3,6,28)); %[1e4,1e5,1e6]; round(logspace(3,6,10)); %
...
NoTot = 3*No;
Sclass = classLevel2*(ones(No*2)-eye(No*2))+eye(No*2);
Sclass(1:No,1:No) = classLevel1*(ones(No)-eye(No))+eye(No);
S = eye(NoTot);
S(1:2*No,1:2*No) = Sclass;


glomRepLarge = makeOdors('correlated',S,NoTot);
...
REx = zeros(length(dSteps), NoTot);
REx2 = zeros(length(dSteps), NoTot);
RExSingle = zeros(length(dSteps), NoTot);
RExSingle2 = zeros(length(dSteps), NoTot);
...
for j=1:length(dSteps)
    ...
    for k=1:length(kSteps)
        ...
        for o=1; %:No
            ...
            rv = full( W*piriformRepLarge );
            rv2 = full( W2*piriformRepLarge2 );
    ...
    save(['pop4thOrderResp_Ex',num2str(popRespIter),'.mat'],'dSteps','kSteps','N4','r','rCC','gCC','rCCpSum','rCCSingle','rAgr','rAgrSingle','rAgrSum','RExSingle','RExSingle2','REx','REx2','Nsingle');
