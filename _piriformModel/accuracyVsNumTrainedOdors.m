...
dSteps = round(logspace(0,7,50)); %10000;
dSteps = dSteps(22:36);
ntSteps = 2.^(0:5);
N4 = 1; %200;
iters = 1; %10;
...
rCC = zeros(length(dSteps), length(ntSteps), No, iters); %zeros(Ny/dStep,dimIters);
rCCpSum = zeros(length(dSteps), length(ntSteps), No, N4, iters); %zeros(Ny/dStep,dimIters);
rCCSingle = zeros(length(dSteps), length(ntSteps), No, iters); %zeros(Ny/dStep,dimIters);
rAcc = zeros(length(dSteps), length(ntSteps), No, iters); %zeros(Ny/dStep,dimIters);

rAgr = zeros(length(dSteps), length(ntSteps), No, iters); %zeros(Ny/dStep,dimIters);
rAgr90 = zeros(length(dSteps), length(ntSteps), No, iters); %zeros(Ny/dStep,dimIters);
rAgrSum = zeros(length(dSteps), length(ntSteps), No, N4, iters); %zeros(Ny/dStep,dimIters);
rAgrSingle = zeros(length(dSteps), length(ntSteps), No, iters); %zeros(Ny/dStep,dimIters);

T50base = zeros(length(dSteps), length(ntSteps), No, 2, iters); %zeros(Ny/dStep,dimIters);
T90base = zeros(length(dSteps), length(ntSteps), No, 2, iters); %zeros(Ny/dStep,dimIters);
agr90base = .9^2+.1^2;

snr = zeros(length(dSteps), length(ntSteps), No, iters); %zeros(Ny/dStep,dimIters);

ZCat = zeros(length(dSteps), length(ntSteps), No*(No-1));
Z2Cat = zeros(length(dSteps), length(ntSteps), No*(No-1));
...
for i=1:iters
    for j=1:length(dSteps)
        for k=1:length(ntSteps)
            display([j/length(dSteps),k/length(ntSteps)])
            ...
            for o=1:No
                trainedOdors = randperm(No,ntSteps(k));
                untrainedOdors = ~ismember(1:No,trainedOdors);
                
                % train readout to one odor
                vTot = -1/(No-ntSteps(k)) * ones( N4, No );
                vTot(:,untrainedOdors) = 1/ntSteps(k);
                W = mean(piriformRepLarge(:,trainedOdors),2)';
                W2 = mean(piriformRepLarge2(:,trainedOdors),2)';
                ...
                z = full( W*piriformRepLarge(:,untrainedOdors) );
                z2 = full( W2*piriformRepLarge2(:,untrainedOdors) );
                ...
                rAcc(j,k,o,i) = sum( Z .* vTot(1,untrainedOdors) < 0 ,2)/(No-1);
                ...
                snr(j,k,o,i) = sum(sum(z))^2 / var(reshape(z,N4*(No-ntSteps(k)),1));
        ...
        save(['popRespVsSparsenss_multiTrain_',num2str(oCorr),'_iter_',num2str(popRespIter),'.mat'],'ZCat','Z2Cat','agr90base','T50base','T90base','rAgr90','oCorr','dSteps','ntSteps','N4','rAcc','rCC','rCCpSum','rCCSingle','rAgr','rAgrSingle','rAgrSum','RExSingle','REx','REx2','RExU','REx2U','snr');
...
if makePlots
    ...
    plot( ntSteps, mean(mean(snr,4),3)/max(mean(mean(snr,4),3)), 'linewidth',2, 'color',[.8,.8,.8]); hold on
    plot( ntSteps, mean(mean(rCC,4),3)/max(mean(mean(rCC,4),3)), '--','linewidth',1, 'color',[.2,.2,.2]); hold on
    plot( ntSteps, mean(mean(rAcc,4),3)/max(mean(mean(rAcc,4),3)), 'linewidth',1, 'color',[.8 0 .4] );
    rAgrmeanNorm = mean(mean(rAgr,4),3);
    rAgrmeanNorm = 2*(rAgrmeanNorm - 0.5);
    plot( ntSteps, rAgrmeanNorm/max(rAgrmeanNorm), 'linewidth',1, 'color',[.6 .3 0] );
    plot( ntSteps, 1/(1-agr90base)*(-agr90base+mean(mean(rAgr90,4),3)/max(mean(mean(rAgr90,4),3))), '--', 'linewidth',1, 'color',[.9 .6 0] )
    box off
