...
dSteps = unique(round(logspace(0,4.5,50)));%round(logspace(3,7,29)); %round(logspace(3,6.69,28)); %round(logspace(3,6,28)); %round(logspace(2,4,10)); %
dSteps(24) = 333;
dSteps(28) = 666;
dSteps(30) = 1000;
dSteps(33) = 2000;
...
cutPt = 13;
rCC(cutPt+1:length(dSteps), :, :) = NaN; 
rAcc(cutPt+1:length(dSteps), :, :) = NaN; 
rAgr(cutPt+1:length(dSteps), :, :) = NaN; 
rAgr90(cutPt+1:length(dSteps), :, :) = NaN; 
snr(cutPt+1:length(dSteps), :, :) = NaN; 
...
REx = zeros(length(dSteps), 1);
REx2 = zeros(length(dSteps), 1);
RExSingle = zeros(length(dSteps), 1);
RExSingle2 = zeros(length(dSteps), 1);
RExU = zeros(length(dSteps), 1);
REx2U = zeros(length(dSteps), 1);
...
for j=1:cutPt
    ...
    save(['flyPop4thOrderResp_aHebb_sm_',num2str(oCorr),'_',num2str(Sc),'_iter_',num2str(popRespIter),'.mat'],'agr90base','T50base','T90base','rAgr90','oCorr','dSteps','kSteps','N4','rAcc','rCC','gCC','rCCpSum','rCCSingle','rAgr','rAgrSingle','rAgrSum','RExSingle','REx','REx2','RExU','REx2U','snr');
